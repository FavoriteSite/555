...
const links = document.querySelectorAll('.pricelist__link[data-name]');
if (links.length > 0) {
  for (let i = 0; i < links.length; i++) {
    const imageLink = links[i];
    imageLink.addEventListener('click', showName);
  }

  function showName(e) {
    const linkActive = e.currentTarget;
    if (linkActive.dataset.name && document.querySelector(linkActive.dataset.name)) {
      const descriptionItem = document.querySelector(linkActive.dataset.name);

      console.log(descriptionItem);
      const itemActive = document.querySelector('.pricelist__list-two._open');
      console.log(itemActive);
      if (itemActive) {
        itemActive.classList.remove('_open');
        descriptionItem.classList.add('_open');
      }
    }
  }
}
